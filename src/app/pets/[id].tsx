...
import { addDoc, collection, onSnapshot, serverTimestamp, setDoc, updateDoc } from "firebase/firestore";
import { SafeAreaView } from "react-native-safe-area-context";
import { useAuth } from "@/src/contexts/AuthContext";
import { db } from "@/firebaseConfig";
import { PetLocationMap } from "@/src/components/PetLocationMap.native";
import { decodeBase64Image } from "@/src/utils/petImages";
import { notifyAdoptionCanceled, notifyAdoptionRequest } from "@/src/services/notifications";
...
function hasHealthFlag(items: string[] | undefined, terms: string[]) {
  return terms.some((term) => (items ?? []).some((item) => item.trim().toLowerCase() === term));
}

function resolveProfileName(profile: UserProfileDocument | null, fallback: string) {
  return profile?.username?.trim() || profile?.name?.trim() || profile?.email?.split("@")[0] || fallback;
}
...
async function geocodeOwnerLocation({ address, city, state }: { address?: string; city?: string; state?: string }) {
  const candidates = [
...
export default function PetDetailScreen() {
  ...
  const contentWidth = useMemo(() => Math.min(width, CONTENT_MAX_WIDTH), [width]);
  ...
  useEffect(() => {
    ...
    async function loadPet() {
      ...
      try {
        ...
        const ownerSnapshot = animalData.usuarioId ? await getDoc(doc(db, "users", animalData.usuarioId)) : null;
        const ownerData = ownerSnapshot?.exists() ? (ownerSnapshot.data() as UserProfileDocument) : null;
        ...
        const imageSourcesFromPhotos = (animalData.fotos ?? [])
          .map((photo) => {
            ...
            return {
              uri: decodeBase64Image(photo.base64, photo.mimeType ?? "image/jpeg"),
            };
          })
        ...
        const temperamentos = (animalData.temperamentos ?? []).map((item) => item.trim()).filter(Boolean);
        ...
        if (isActive) {
          const nextPet = {
            ...
            localizacao: city && state ? `${city} - ${state}` : "Localização não informada",
            castrado: hasHealthFlag(healthItems, ["castrado", "castrada"]) ? "Sim" : "Não",
            vermifugado: hasHealthFlag(healthItems, ["vermifugado", "vermifugada"]) ? "Sim" : "Não",
            vacinado: hasHealthFlag(healthItems, ["vacinado", "vacinada"]) ? "Sim" : "Não",
            doencas: animalData.doencas?.trim() || "Nenhuma informada",
            temperamento: temperamentos.length > 0 ? temperamentos.join(", ") : "Não informado",
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const conversation = snapshot.exists() ? (snapshot.data() as { adoptionRequestActive?: boolean }) : null;
  ...
  const handleChangeImage = useCallback(
    (direction: "prev" | "next") => {
      ...
      const nextIndex =
        direction === "next" ? Math.min(currentImageIndex + 1, lastIndex) : Math.max(currentImageIndex - 1, 0);
  ...
  const handleAdopt = useCallback(async () => {
    ...
    const interestedUserName =
      profile?.username?.trim() || profile?.name?.trim() || user.email?.split("@")[0] || "Alguém";
    const conversationId = `${pet.id}_${user.uid}`;
  ...
  const adoptionButtonLabel = hasAdoptionRequest ? "DESISTO DA ADOÇÃO" : "PRETENDO ADOTAR";
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.screen}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{pet?.nome ?? "Detalhes do pet"}</Text>
        ...
        <ScrollView bounces={false} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          {isLoading ? (
            <View style={[styles.contentContainer, styles.feedbackContainer, { width: contentWidth }]}>
            ...
            <View style={[styles.contentContainer, { width: contentWidth }]}>
              <View style={styles.imageSection}>
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <Image source={item} resizeMode="cover" style={[styles.heroImage, { width: contentWidth }]} />
                  )}
                />

                <PaginationDots total={pet.imagens.length} currentIndex={currentImageIndex} />
                  ...
                  <>
                    ...
                    <Pressable
                      ...
                      style={[
                        ...
                        currentImageIndex === pet.imagens.length - 1 ? styles.carouselButtonDisabled : null,
                      ]}
                    >
                ...
                {!isOwnPet ? (
                  <Pressable
                    accessibilityRole="button"
                    accessibilityLabel={
                      hasAdoptionRequest ? `Desistir da adoção de ${pet.nome}` : `Pretendo adotar ${pet.nome}`
                    }
                    onPress={handleToggleFavorite}
                    style={styles.favoriteFab}
                  >
                    <MaterialIcons name={isFavorite ? "favorite" : "favorite-border"} size={24} color="#434343" />
                  </Pressable>
                ) : null}
              </View>
              ...
              <View style={styles.infoContainer}>
                ...
                <InfoBlock label="EXIGÊNCIAS DO DOADOR" value={pet.exigenciasDoador} />

                <Divider />

                <InfoBlock label={`MAIS SOBRE ${pet.nome.toUpperCase()}`} value={pet.descricao} />
                ...
                <View style={styles.ctaWrapper}>
                  <Pressable
                    ...
                    style={[styles.ctaButton, isOwnPet ? styles.ctaButtonDisabled : null]}
                  >
                    <Text style={[styles.ctaButtonText, isOwnPet ? styles.ctaButtonTextDisabled : null]}>
                      {adoptionButtonLabel}
                    </Text>
                  </Pressable>
                </View>
        ...
        <Modal animationType="fade" transparent visible={isAuthPromptVisible} onRequestClose={handleCloseAuthPrompt}>
            ...
            <View style={styles.authPrompt}>
              ...
              <View style={styles.authPromptActions}>
                <Pressable
                  ...
                  style={({ pressed }) => [styles.authPromptPrimaryButton, pressed && styles.authPromptButtonPressed]}
                >
                ...
                <Pressable
                  ...
                  style={({ pressed }) => [styles.authPromptSecondaryButton, pressed && styles.authPromptButtonPressed]}
                >
...
const PaginationDots = memo(function PaginationDots({ total, currentIndex }: PaginationDotsProps) {
  return (
