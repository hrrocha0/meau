...
import { useNearbyPets, PetPin } from "@/src/hooks/usaPetsproximos";
import { colors } from "@/constants";
import { GlobalPetsMap } from "@/src/components/GlobalPetsMap.native";
...
export default function MapaGlobal() {
  ...
  return (
    <View style={styles.container}>
      <GlobalPetsMap pets={pets} userLocation={userLocation} onSelectPet={setSelecionado} />
      ...
      {selecionado && (
        <View style={styles.bottomSheet}>
          {selecionado.imagemUri && <Image source={{ uri: selecionado.imagemUri }} style={styles.sheetImg} />}
          <View style={styles.sheetInfo}>
            <Text style={styles.sheetNome}>{selecionado.nome}</Text>
            <Text style={styles.sheetEspecie}>{selecionado.especie}</Text>
          </View>
          <View style={styles.sheetBtns}>
            <Pressable style={styles.btnVer} onPress={() => router.push(`/pets/${selecionado.id}`)}>
