...
export default function AdotarScreen() {
    ...
    async (isActive: () => boolean) => {
      try {
        ...
        const [ownerEntries, favoriteSnapshot] = await Promise.all([
          Promise.all(
            ([...new Set(animals.map((animal) => animal.usuarioId).filter(Boolean))] as string[]).map(
              async (ownerId) => {
                const ownerSnapshot = await getDoc(doc(db, "users", ownerId));
                return [
                  ownerId,
                  ownerSnapshot.exists() ? (ownerSnapshot.data() as UserProfileDocument) : null,
                ] as const;
              },
            ),
          ),
          user?.uid ? getDocs(collection(db, "users", user.uid, "favorites")).catch(() => null) : Promise.resolve(null),
        ]);
        ...
        const nextPets = animals
          ...
          .map((animal) => {
            ...
            const localizacao =
              city && state ? `${city.toUpperCase()} - ${state.toUpperCase()}` : "LOCALIZAÇÃO NÃO INFORMADA";
            ...
            const imagemUris =
              imageUrisFromPhotos.length > 0 ? imageUrisFromPhotos : animal.fotoUrl ? [animal.fotoUrl] : [];
    ...
    async (pet: Pet) => {
      ...
      setPets((currentPets) =>
        currentPets.map((currentPet) =>
          currentPet.id === pet.id ? { ...currentPet, favorito: nextFavorite } : currentPet,
        ),
      );
      ...
      try {
        ...
        setPets((currentPets) =>
          currentPets.map((currentPet) =>
            currentPet.id === pet.id ? { ...currentPet, favorito: pet.favorito ?? false } : currentPet,
          ),
        );
  ...
  const renderItem = useCallback(
    ({ item }: ListRenderItemInfo<Pet>) => (
      <PetCard pet={item} width={cardWidth} onPress={handleOpenPet} onToggleFavorite={handleToggleFavorite} />
    ),
...
const PetCard = memo(function PetCard({ pet, width, onPress, onToggleFavorite }: PetCardProps) {
  ...
  const imageUris = pet.imagemUris.length > 0 ? pet.imagemUris : [""];
  ...
  return (
    <View style={[styles.cardWrapper, { width, minHeight: CARD_TOTAL_HEIGHT }]}>
      <View style={styles.cardHeader}>
        ...
        >
          <MaterialIcons name={pet.favorito ? "favorite" : "favorite-border"} size={24} color="#434343" />
        </Pressable>
      </View>
            ...
            <View style={styles.carouselDots}>
              {imageUris.map((_, index) => (
                <View
                  ...
                  style={[styles.carouselDot, index === currentImageIndex ? styles.carouselDotActive : null]}
                />
              ))}
            </View>
          </>
