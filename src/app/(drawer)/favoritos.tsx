...
export default function FavoritosScreen() {
  ...
  const loadFavoritePets = useCallback(
    async (isActive: () => boolean) => {
      ...
      try {
        ...
        const animals = animalEntries.filter(
          (animal): animal is AnimalDocument & { id: string } => Boolean(animal) && animal?.oculto !== true,
        );
  ...
  const renderItem = useCallback(
    ({ item }: ListRenderItemInfo<Pet>) => (
      <PetCard pet={item} width={cardWidth} onPress={handleOpenPet} onRemoveFavorite={handleRemoveFavorite} />
    ),
  ...
  const emptyText = user ? "Nenhum pet favorito ainda." : "Entre na sua conta para ver seus favoritos.";
