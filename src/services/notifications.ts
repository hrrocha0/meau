...
function isNativeFirebaseMissingError(error: unknown) {
  return error instanceof Error && error.message.includes("Default FirebaseApp is not initialized");
}
...
async function handleAdoptionNotificationAction(actionIdentifier: AdoptionNotificationAction, data: NotificationData) {
...
export async function registerPushTokenForUser(userId: string) {
  ...
  try {
    ...
    const tokenRef = doc(db, "users", userId, "pushTokens", buildTokenDocumentId(expoPushToken));
...
async function registerWebPushTokenForUser(userId: string) {
  ...
  await setDoc(
    doc(db, "users", userId, "pushTokens", buildTokenDocumentId(token)),
    {
      token,
      platform: "web",
      provider: "fcm-web",
...
export async function unregisterPushTokenForUser(userId: string, expoPushToken: string | null) {
  ...
  await deleteDoc(doc(db, "users", userId, "pushTokens", buildTokenDocumentId(expoPushToken)));
}
...
async function notifyUser(recipientUserId: string, notification: NotificationPayload) {
  try {
    if (Platform.OS === "web") {
